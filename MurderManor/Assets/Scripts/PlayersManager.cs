...
public class PlayersManager : MonoBehaviour {
   ...
   private void Start() {
      _grpc_channel = new Grpc.Core.Channel(endpoint,
   ...
   public void MovePlayer(string id, UnityEngine.Vector2 position, UnityEngine.Vector2 direction) {
      _client.MovePlayer(new MovePlayerRequest {
            Id = id,
            NewCoordinates = new Gameapi.Vector2{
               X = position.x,
               Y = position.y,
            },
            NewDirection = new Gameapi.Vector2{
               X = direction.x,
               Y = direction.y,
            },
            });
